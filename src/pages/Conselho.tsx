...
export default function Conselho() {
    ...
    return (
        <div className="p-6 max-w-5xl mx-auto space-y-12 pb-12">
            <PageHeader title="Conselho Administrivo" subtitle="Gestão estratégica de participantes, diretrizes e registros de reuniões" />
            ...
            <section className="space-y-6">
                <div className="flex justify-between items-center border-b-2 border-indigo-600/10 pb-4">
                    <h2 className="text-2xl font-black flex items-center gap-3 text-indigo-600 uppercase tracking-tight">
                        <Users className="w-6 h-6" /> Membros do Conselho
                    </h2>
                    {isAdmin && (
            ...
            <section className="space-y-6">
                <div className="flex justify-between items-center border-b-2 border-emerald-600/10 pb-4">
                    <h2 className="text-2xl font-black flex items-center gap-3 text-emerald-600 uppercase tracking-tight">
                        <BookOpen className="w-6 h-6" /> Diretrizes e Planejamento
                    </h2>
                    {isAdmin && (
                        <Dialog open={isPlanejamentoModalOpen} onOpenChange={(open) => { setIsPlanejamentoModalOpen(open); if (!open) setEditingPlanejamento(null); }}>
                            <DialogTrigger asChild>
                                <Button className="bg-emerald-600 hover:bg-emerald-700 h-11 px-6 rounded-xl font-bold shadow-lg shadow-emerald-600/20">
            ...
            <section className="space-y-6">
                <div className="flex justify-between items-center border-b-2 border-amber-600/10 pb-4">
                    <h2 className="text-2xl font-black flex items-center gap-3 text-amber-600 uppercase tracking-tight">
                        <Calendar className="w-6 h-6" /> Atas de Reuniões
                    </h2>
                    {isAdmin && (
                        <Dialog open={isReuniaoModalOpen} onOpenChange={(open) => { 
                            ...
                            <DialogTrigger asChild>
                                <Button className="bg-amber-600 hover:bg-amber-700 h-11 px-6 rounded-xl font-bold shadow-lg shadow-amber-600/20">
                    ...
                    {reunioes.length === 0 ? (
                        ...
                        reunioes.sort((a,b) => new Date(b.data).getTime() - new Date(a.data).getTime()).map((r, i) => (
                            <motion.div
                                ...
                                className="bg-white/60 backdrop-blur-md border-l-[12px] border-l-amber-500 rounded-3xl p-8 relative group hover:shadow-2xl transition-all border border-amber-100/50"
                            >
                                <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-6">
                                    <div>
                                        ...
                                        <p className="text-3xl font-black text-amber-950 uppercase tracking-tighter leading-none">
                                            {r.topicos}
                                        </p>
                                    </div>
                                    <Button variant="outline" size="sm" className="h-10 px-4 gap-2 text-amber-700 border-amber-200 bg-amber-50 hover:bg-amber-100 rounded-xl font-bold" onClick={() => generatePDF(r)}>
                                        <Printer className="w-5 h-5" /> GERAR PDF DA ATA
                                    </Button>
                                </div>
